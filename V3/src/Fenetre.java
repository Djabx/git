...
public class Fenetre extends JFrame implements ActionListener, KeyListener {
	...
	public Fenetre() throws ImageProcessingException, IOException {
		...
		this.setSize(outil.getScreenSize()); // ouvrir en pleins ecran en
												// fcontion de l'ordi mais
												// pouvoir modifier taille
			...
			if (valeur == JFileChooser.APPROVE_OPTION) {
				...
				try {
					...
					for (GpsDirectory gpsDirectory : gpsDirectories) {
						 ...
						 * gpsDesc.getGpsLatitudeDescription());
						 * System.out.println("Longitude : " +
						 * gpsDesc.getGpsLongitudeDescription());
						 * System.out.println("Date : " +
						 * gpsDesc.getGpsTimeStampDescription());
						 * System.out.println("Minute  : " +
						 * gpsDesc.getDegreesMinutesSecondsDescription());
						 */

						System.out.println("Latitude:" + lat + " " + "Longitude:" + longe);
						JLabel monJLabel = new JLabel(); // on crée une image a
															// partir du chemin
															// de l'image
															// importé de taille
															// 80 dans un jpanel
															// qu'on ajoute a la
															// map
						ImageIcon icon = new ImageIcon(new ImageIcon(gf.getSelectedFile().getAbsolutePath()).getImage()
								.getScaledInstance(80, 80, Image.SCALE_DEFAULT));

						monJLabel.setIcon(icon);
						GeoPosition posi = new GeoPosition(lat, longe);
						monJLabel.setLocation((int) m.convertGeoPositionToPoint(posi).getX(),
								(int) m.convertGeoPositionToPoint(posi).getY());
